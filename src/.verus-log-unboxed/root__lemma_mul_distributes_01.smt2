...
;; MODULE ''
;; NonLinearArith/dist_unboxed.rs:124:7: 124:33 (#0)
...
;; Function-Def dist_unboxed::lemma_mul_distributes
(push)
 (declare-const no%param@ Int)
 (declare-const x~92@ Poly)
 (declare-const y~94@ Poly)
 (declare-const z~96@ Poly)
 (declare-const tmp%1@ Bool)
 (declare-const f2~186@ %%Function%%)
 (assert
  fuel_defaults
 )
 (declare-fun %%lambda%%0 (Poly Poly Poly Poly) %%Function%%)
 (assert
  (forall ((%%hole%%0 Poly) (%%hole%%1 Poly) (%%hole%%2 Poly) (%%hole%%3 Poly) (i~159$
     Poly
    )
   ) (!
    (= (%%apply%%0 (%%lambda%%0 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3) i~159$) (B (= (
        dist_unboxed!dist_left_sub.? %%hole%%0 i~159$ %%hole%%1
       ) (dist_unboxed!dist_right_sub.? %%hole%%2 i~159$ %%hole%%3)
    )))
    :pattern ((%%apply%%0 (%%lambda%%0 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3) i~159$))
 )))
     ...
     (=>
      ...
      (=>
       ...
       (=>
        (has_type z~96@ INT)
        (=>
         (= f2~186@ (mk_fun (%%lambda%%0 x~92@ z~96@ x~92@ z~96@)))
         (and
          (=>
           %%location_label%%0
           (req%dist_unboxed!lemma_mul_induction. f2~186@)
          )
          (=>
           (ens%dist_unboxed!lemma_mul_induction. f2~186@)
           (=>
            (= tmp%1@ (%B (%%apply%%0 f2~186@ y~94@)))
            (and
             (=>
              %%location_label%%1
              tmp%1@
             ...
             (=>
              ...
              (=>
               %%location_label%%2
               (and
                (= (dist_unboxed!dist_left_add.? x~92@ y~94@ z~96@) (dist_unboxed!dist_right_add.?
                  x~92@ y~94@ z~96@
                ))
                (= (dist_unboxed!dist_left_sub.? x~92@ y~94@ z~96@) (dist_unboxed!dist_right_sub.?
                  x~92@ y~94@ z~96@
     )))))))))))))
     (=>
      (forall ((x~92$ Poly) (y~94$ Poly) (z~96$ Poly)) (!
        (=>
         (and
          (has_type x~92$ INT)
          (has_type y~94$ INT)
        ...
        :pattern ((dist_unboxed!dist_left_add.? x~92$ y~94$ z~96$) (dist_unboxed!dist_left_sub.?
          x~92$ y~94$ z~96$
        ))
        :qid user_dist_unboxed__lemma_mul_distributes_29
        :skolemid skolem_user_dist_unboxed__lemma_mul_distributes_29
      ))
      (and
       (=>
        %%location_label%%3
        (forall ((x~14$ Poly) (y~16$ Poly) (z~18$ Poly)) (!
          ...
          :pattern ((dist_unboxed!dist_left_add.? x~14$ y~16$ z~18$))
          :qid user_dist_unboxed__lemma_mul_distributes_27
          :skolemid skolem_user_dist_unboxed__lemma_mul_distributes_27
       )))
       (=>
        %%location_label%%4
        (forall ((x~52$ Poly) (y~54$ Poly) (z~56$ Poly)) (!
          ...
          :pattern ((dist_unboxed!dist_left_sub.? x~52$ y~54$ z~56$))
          :qid user_dist_unboxed__lemma_mul_distributes_28
          :skolemid skolem_user_dist_unboxed__lemma_mul_distributes_28
 )))))))))
 (get-info :version)
 (assert
  %%query%%
