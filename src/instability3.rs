// deep water
// mod basics, cannot prove the auto version

// UPDATE: Jay minimizes this too z3 wierdness
    ...
    ensures (x + n) % n == x % n
{
    ...
    assert(0 == n * zp + ((x + n) % n) - (x % n)) by { 
        // UNCOMMENT THIS PART IF ASSERT FAILS
        // assert (n * zp == n * (((x + n) / n - x / n) - 1));
        // assert (n * zp == n * ((x + n) / n - x / n) - n) by {
        //     assert( n * (((x + n) / n - x / n) - 1) == n * ((x + n) / n - x / n) - n) by {
        //         lemma_mul_is_distributive_auto();
    ...
    // this line needs to be added even when copying over from the original files
...
// this function just cannot be proven 
