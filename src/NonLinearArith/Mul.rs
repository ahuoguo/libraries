...
// proof fn lemma_mul_nonzero_auto()
//     ensures forall |x: int, y: int| #[trigger] x * y != 0 <==> x != 0 && y != 0
// {
//     forall (x: int, y: int)    //     ensures x * y != 0 <==> x != 0 && y != 0;
//     {
...
// proof fn lemma_mul_by_zero_is_zero_auto()
//     ensures forall |x: int| #[trigger]mul(x, 0) == #[trigger]mul(0, x) == 0
// {
...
// proof fn lemma_mul_equality_auto()
...
// proof fn lemma_mul_inequality_auto()
...
// proof fn lemma_mul_strict_inequality_auto()
//     ensures  forall x: int, y: int, z: int {:trigger x * z, y * z} :: x < y && z > 0 ==> x * z < y * z
// {
//     forall (x: int, y: int, z: int | x < y && z > 0)
//     ensures x * z < y * z
...
// proof fn lemma_mul_upper_bound_auto()
...
// proof fn lemma_mul_strict_upper_bound_auto()
...
//   proof fn lemma_mul_left_inequality_auto()
...
// /// two original integers are equal
// proof fn lemma_mul_equality_converse_auto()
...
// proof fn lemma_mul_inequality_converse_auto()
...
// /// when two integers, x and y, are each multiplied by a positive integer, z, if x < z then the x*z < y*z 
// /// for all valid values of x, y, and z
// proof fn lemma_mul_strict_inequality_converse_auto()
//     ensures  forall x: int, y: int, z: int {:trigger x * z, y * z} :: x * z < y * z && z >= 0 ==> x < y
// {
//     forall (x: int, y: int, z: int | x * z < y * z && z >= 0)
//     ensures x < y;
...
//     ensures
//         forall x: int, y: int, z: int {:trigger x * (y + z)} :: x * (y + z) == x * y + x * z,
//         forall x: int, y: int, z: int {:trigger x * (y - z)} :: x * (y - z) == x * y - x * z,
//         forall x: int, y: int, z: int {:trigger (y + z) * x} :: (y + z) * x == y * x + z * x,
//         forall x: int, y: int, z: int {:trigger (y - z) * x} :: (y - z) * x == y * x - z * x,
// {
        ...
        (-x) * y == -(x * y) && -(x * y) == x * (-y)
{
    ...
    assert(mul_auto() ==> forall |i| #[trigger] is_le(0, i) && f(i) ==> f(i + 1));
    // assert (mul_auto() ==> { &&&  f(0)
    //     &&& (forall |i| #[trigger] is_le(0, i) && f(i) ==> f(i + 1))
    //     &&& (forall |i| #[trigger] is_le(i, 0) && f(i) ==> f(i - 1))}); // OBSERVE
...
//     forall x: int, y: int, z: int {:trigger x * z, y * z} :: x < y && z > 0 ==> x * z < y * z,
//     forall x: int, y: int, z: int {:trigger x * z, y * z} :: x <= y && z >= 0 ==> x * z <= y * z,
//     forall x: int, y: int, z: int {:trigger x * (y + z)} :: x * (y + z) == x * y + x * z,
//     forall x: int, y: int, z: int {:trigger x * (y - z)} :: x * (y - z) == x * y - x * z,
//     forall x: int, y: int, z: int {:trigger (y + z) * x} :: (y + z) * x == y * x + z * x,
...
//     forall x: int, y: int {:trigger x * y} :: x * y != 0 <==> x != 0 && y != 0,
//     forall x: int, y: int {:trigger x * y} :: 0 <= x && 0 <= y ==> 0 <= x * y,
//     forall x: int, y: int {:trigger x * y} :: 0 < x && 0 < y && 0 <= x * y ==> x <= x * y && y <= x * y,
//     forall x: int, y: int {:trigger x * y} :: (1 < x && 0 < y) ==> (y < x * y),
//     forall x: int, y: int {:trigger x * y} :: (0 < x && 0 < y) ==> (y <= x * y),
//     forall x: int, y: int {:trigger x * y} :: (0 < x && 0 < y) ==> (0 < x * y),
// {
